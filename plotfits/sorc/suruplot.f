...
c
       ...
       real(8) sprs(nreg,nsub,nlev,nstcw,nvar,ntx)
       real(8),allocatable,dimension(:)            :: xcnt,xrms,xavg

       real(4) cnt(nreg,nsub,nlev)
       real(4) avg(nreg,nsub,nlev)
       real(4) rms(nreg,nsub,nlev)
...
c
      ...
      xcnt(nt) = sprs(ireg,isub,ilev,1,ivar,nt)
      xavg(nt) = sprs(ireg,isub,ilev,2,ivar,nt)
      xrms(nt) = sprs(ireg,isub,ilev,3,ivar,nt)

      enddo

      ! add the ensemble count, rmse, and, mean

      cnt(ireg,isub,ilev) = sum(xcnt(:))/float(ntime)
      if(sum(xcnt(:))>0.) then 
       avg(ireg,isub,ilev) = sum(xavg(:)*xcnt(:))/sum(xcnt(:))
       rms(ireg,isub,ilev) = sqrt(sum(xrms(:)**2*xcnt(:))/sum(xcnt(:)))
      endif

      if(ireg==nreg) then
         if(ivar>2) print*,ilev 
         if(ivar>2) print*,cnt(:,isub,ilev)
         if(ivar>2) print*,avg(:,isub,ilev)
         if(ivar>2) print*,rms(:,isub,ilev)
      endif

      enddo; enddo; enddo

      call wrtstat(51,cnt,nreg,nsub,nlev)
      call wrtstat(51,cnt,nreg,nsub,nlev)
      call wrtstat(51,cnt,nreg,nsub,nlev)
      call wrtstat(51,cnt,nreg,nsub,nlev)
      call wrtstat(51,cnt,nreg,nsub,nlev)
      ...
      if(ivar==3) then 
         call wrtstat(51,cnt,nreg,nsub,nlev)
         call wrtstat(51,cnt,nreg,nsub,nlev)
         call wrtstat(51,avg,nreg,nsub,nlev)
         call wrtstat(51,rms,nreg,nsub,nlev)
      else  
         call wrtstat(51,rms,nreg,nsub,nlev)
         call wrtstat(51,avg,nreg,nsub,nlev)
      endif
