...
if [ ${exp} = "fnc" -o ${exp} = "fnx" -o ${exp} = "gfs" -o ${exp} = "ecm" -o ${exp} = "cdas" -o ${exp} = "avn" -o ${exp} = "gdas" -o ${exp} = "ukm" ]; then
 if [ $exp = $exp1 ]; then
...
$SUBJOB -e $listvar -a $task  -q $cue -g $GROUP -p 1/1 -t 3:00:00 -o $PROUT/vertplot_$exp.out \
...
$SUBJOB -e $listvar -a $task  -q $cue -g $GROUP -p 1/1 -t 3:00:00 -o $PROUT/vertplot_$sdir.out \
...
if [ $exp = timeout ]; then
 ...
 export listvar=quan,edate,edate12,sdate,sdate12,var,exp1,exp2,pdir,web,webdir,localdir,SCRIPTS,GSCRIPTS,ctldir,namstr,webmch,webid,GRADSBIN,IMGCONVERT
 $SUBJOB -e $listvar -a $task  -q $cue -g $GROUP -p 1/1 -t 3:00:00 -o $PROUT/timeout.newb.out $SCRIPTS/timeout.newb
 continue
fi
...
if [ $exp = timevrt ]; then
 export pdir=$ptdir;  mkdir -p $pdir; echo "exp is $exp"
 export listvar=quan,edate,edate12,sdate,sdate12,var,exp1,exp2,pdir,web,webdir,localdir,SCRIPTS,GSCRIPTS,ctldir,namstr,webmch,webid,GRADSBIN,IMGCONVERT
 $SUBJOB -e $listvar -a $task  -q $cue -g $GROUP -p 1/1 -t 3:00:00 -o $PROUT/timevrt.newb.out $SCRIPTS/timevrt.newb
 continue
fi
...
export listvar=edate,edate12,sdate,sdate12,var,exp,pdir,web,webdir,localdir,SCRIPTS,GSCRIPTS,ctldir,namstr,webmch,webid,GRADSBIN,stnmap,IMGCONVERT
$SUBJOB -e $listvar -a $task  -q $cue -g $GROUP -p 1/1 -t 3:00:00 -o $PROUT/time${var}_$exp.out \
$SCRIPTS/time${var}.newb
done

export pdir=$ptdir/acar; mkdir -p $pdir
export listvar=edate,edate12,sdate,sdate12,var,exp,pdir,web,webdir,localdir,SCRIPTS,GSCRIPTS,ctldir,namstr,webmch,webid,GRADSBIN,stnmap,IMGCONVERT
$SUBJOB -e $listvar -a $task  -q $cue -g $GROUP -p 1/1 -t 3:00:00 -o $PROUT/timeacar_$exp.out \
$SCRIPTS/timeacar.newb

export pdir=$ptdir/acft;  mkdir -p $pdir
export listvar=edate,edate12,sdate,sdate12,var,exp,pdir,web,webdir,localdir,SCRIPTS,GSCRIPTS,ctldir,namstr,webmch,webid,GRADSBIN,stnmap,IMGCONVERT
$SUBJOB -e $listvar -a $task  -q $cue -g $GROUP -p 1/1 -t 3:00:00 -o $PROUT/timeacft_$exp.out \
...
$SUBJOB -e $listvar -a $task  -q $cue -g $GROUP -p 1/1 -t 3:00:00 -o $PROUT/time${var}_$sdir.out \
...
export listvar=edate,edate12,sdate,sdate12,var,exp1,exp2,fcs1,fcs2,pdir,web,webdir,localdir,SCRIPTS,GSCRIPTS,ctldir,namstr,webmch,webid,GRADSBIN,stnmap,IMGCONVERT
$SUBJOB -e $listvar -a $task  -q $cue -g $GROUP -p 1/1 -t 3:00:00 -o $PROUT/timeacar_$sdir.out \
...
export listvar=edate,edate12,sdate,sdate12,var,exp1,exp2,fcs1,fcs2,pdir,web,webdir,localdir,SCRIPTS,GSCRIPTS,ctldir,namstr,webmch,webid,GRADSBIN,stnmap,IMGCONVERT
$SUBJOB -e $listvar -a $task  -q $cue -g $GROUP -p 1/1 -t 3:00:00 -o $PROUT/timeacft_$sdir.out  \
...
$SUBJOB -e $listvar -a $task  -q $cue -g $GROUP -p 1/1 -t 3:00:00 -o $PROUT/f00plots_$exp2.out \
...
export listvar=edate,edate12,sdate,sdate12,minday,exp1dir,exp2dir,exp1,exp2,pdir,GSCRIPTS,SCRIPTS,web,webdir,localdir,namstr,webmch,webid,GRADSBIN,stnmap,IMGCONVERT
$SUBJOB -e $listvar -a $task  -q $cue -g $GROUP -p 1/1 -t 3:00:00 -o $PROUT/f12plots_$exp2.out \
...
$SUBJOB -e $listvar -a $task  -q $cue -g $GROUP -p 1/1 -t 3:00:00 -o $PROUT/horizab_$exp.out \
...
export listvar=edate,edate12,sdate,sdate12,minday,expdir,exp,pdir,GSCRIPTS,SCRIPTS,web,webdir,localdir,namstr,webmch,webid,GRADSBIN,stnmap,IMGCONVERT
$SUBJOB -e $listvar -a $task  -q $cue -g $GROUP -p 1/1 -t 3:00:00 -o $PROUT/horizrab_$exp.out \
...
export listvar=edate,edate12,sdate,sdate12,minday,expdir,exp,pdir,GSCRIPTS,SCRIPTS,web,webdir,localdir,namstr,webmch,webid,GRADSBIN,stnmap,IMGCONVERT
$SUBJOB -e $listvar -a $task  -q $cue -g $GROUP -p 1/1 -t 3:00:00 -o $PROUT/sfcab_$exp.out \
  $SCRIPTS/sfcab.cdas
